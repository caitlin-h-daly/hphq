#' Retrieves credible hierarchies for poset question types
#'
#' @description
...
#'
#' @param effects.matrix A matrix where the column headers are treatment names and each row displays each treatment’s sampled relative effect for that iteration
...
#'
#' @return A data frame containing the credible hierarchies for Posets
#' @importFrom utils combn
#' @export
#'
#' @examples
#' get_posets(effects.matrix,10,0.2,TRUE)
get_posets <- function(effects.matrix,MID,threshold,largerBetter) {
  if(threshold>1 || threshold<0) {
    stop("Please ensure threshold value is between 0 and 1")
  }
  treatments <- colnames(effects.matrix)
  output_list <- vector('list')
  output_list_index <- 1
  n.treatments <- length(treatments)
  n.iterations <- nrow(effects.matrix)
  ...
  Freq <- apply (perms, 1, function(row) {
    ...
    trt2 <- row['V2']
    if (largerBetter) {
      sum((effects.matrix[,trt1] - effects.matrix[,trt2]) > MID_t)/n.iterations
    } else {
      sum((effects.matrix[,trt1] - effects.matrix[,trt2]) < MID_t)/n.iterations
    }
  })
  ...
  # n > 2, loop repeats while there are still permutations left after filtering and permutation size <= number of treatments
  while(total_rows > 0 && perm_size <= n.treatments) {
    ...
    Freq <- apply (perms, 1, function(row) {
      trts <- row[]
      compare_matrix <- matrix(,nrow=n.iterations,ncol=(length(trts)-1))
      for (i in seq (1:(length(trts)-1))) {
        trt1 <- row[i]
        trt2 <- row[i+1]
        if (largerBetter) {
          compare_matrix[,i] <- (effects.matrix[,trt1] - effects.matrix[,trt2]) > MID_t
        } else {
          compare_matrix[,i] <- (effects.matrix[,trt1] - effects.matrix[,trt2]) < MID_t
        }
      }
