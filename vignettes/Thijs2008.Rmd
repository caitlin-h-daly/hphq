...
function. This uses the first algorithm described by Daly et al. (2025).

```{r}
algo1 <- get_arrangements(
  hierarchy_matrix = inputs$hierarchy_matrix,
  threshold = 0.95
)
algo1
```
...
```{r}
algo2 <- get_partial_hierarchies(
  effects_matrix = inputs$effects_matrix,
  mid = 0,
  threshold = 0.95,
  larger_better = FALSE
)
...
# Rankings and highest density region (HDR) sets

To obtain all credible rankings, as well as the highest density region sets, 
for each treatment at a given threshold, you may use the `get_ranks_by_treatment()` 
function. This uses the third algorithm described by Daly et al. (2025).

Note that the rankograms for each treatment may be plotted with overlayed HDR 
sets by setting the `print_plot` argument to TRUE.

```{r}
algo3 <- get_ranks_by_treatment(
  ranking_df = inputs$ranking_df,
  threshold = 0.95,
  print_plot = FALSE
)
algo3
...
`find_redundancies()` to detect these redundant hierarchies and optionally remove
them. By default, `find_redundancies()` detects redundant hierarchies in the 
order proposed by Daly et al. (2025). The user has the option to only check 
certain hierarchy types, in their preferred order. See the help page for more 
details on this.

```{r}
redundant_check <- find_redundancies(
  algo_1 = algo1,
  algo_2 = algo2,
  algo_3 = algo3,
  n_trt = ncol(inputs$effects_matrix),
  threshold = 0.95
)
redundant_check
...
ranked combinations, combinations, partial hierarchies, rankings, and HDR sets) 
...
```{r}
all_cred_hier <- get_all_questions(
  inputs = inputs,
  n_trt = ncol(inputs$effects_matrix),
  larger_better = FALSE,
...
Daly CH, Tan C, Béliveau A. (2026). Uncovering all highly credible binary treatment hierarchy questions in network meta-analysis. Under review.
